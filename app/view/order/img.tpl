...
{$l.goods_name}
 ...
 {if $l.party!=""}
<div class="party">{$l.party} 库存{$l.stock}件</div> 
{/if}
<span class="pull-right" title="累计销量">{$l.sales}{$l.unit}</span>
</div>

</div>
<div class="attribute">
<span class="price">{$l.price*100}积分</span> <span class="market_price"></span> <span class="pull-right view" title="推荐指数{$l.view}"><i class="star"></i></span>
</div>
...
{else}
<a href="javascript:addCart({$l.id})" id="{$l.id}" jsordername="{$l.goods_name}" price="{$l.price*100*$l.num}"  class="btn btn-default jsorderadd">立即兑换</a></td>
{/if}
{/if}
...
function upload(sku){	
	$.layer({
	    ...
	    offset: ['20px',''],
	    area: ['850px', '480px'],
	    iframe: {src: './index.php?buy/upload/?sku='+sku,
	        scrolling: 'yes'
	      }
	}) 
...
var options = {
		...
		pageUrl: function(type, page, current){
		    return './index.php?order/shop/?p='+page+'#content';
		}
		}
